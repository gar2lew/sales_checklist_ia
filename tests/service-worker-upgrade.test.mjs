...
assert.match(source, /const CACHE_VERSION = 'v1\.10\.2';/, "release cache must advance from v1.10.1 to v1.10.2");
...
const context = {
  ...
  caches: {
    ...
    async keys() { return ["sales-capture-v1.10.1", "sales-capture-v1.9.9", "sales-capture-v1.10.2", "unrelated-cache"]; },
...
assert.equal(openedCaches[0], "sales-capture-v1.10.2", "fresh install must populate the new cache");
...
assert.deepEqual(cachedAssets, [
  ...
  "/icons/icon-192.png", "/icons/icon-512.png", "/icons/asg_logo.png", "/icons/asg_logo_white.png",
...
await lifecyclePromise;
assert.deepEqual(deletedCaches, ["sales-capture-v1.10.1", "sales-capture-v1.9.9"], "upgrade must delete previous application caches without touching unrelated caches");
