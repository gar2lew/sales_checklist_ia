...
try {
  ...
  await page.click('#landingStaffMenu li[data-value="Sam"]');
  await page.fill('#landingClient1', 'Alexandria Catherine Montgomery-Wellington');
  await page.fill('#landingClient2', 'Benjamin Theodore Fitzwilliam-Smythe');
  await page.dispatchEvent('#landingForm', 'submit');

  await page.fill('#date', '2026-07-16');
  ...
  for (const testCase of cases) {
    ...
    assert.equal(await page.textContent('#previewPageLabel'), 'Page 1 of 2');
    await page.click('#previewNext');
    await page.waitForFunction(() => document.querySelector('#previewPageLabel')?.textContent === 'Page 2 of 2');
    await page.locator('#previewPaper canvas').screenshot({ path: join(caseDir, 'ia-preview.png') });
