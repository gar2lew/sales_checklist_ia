...
const previousWorker = currentWorker.replace("v2.7.0-alpha.14", "v2.7.0-alpha.13");
...
try {
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.13");
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.14", ["sales-capture-v2.7.0-alpha.13"]);
  ...
  await waitForCache(freshPage, "sales-capture-v2.7.0-alpha.14", ["sales-capture-v2.7.0-alpha.13"]);
  assert.equal(await freshPage.locator("#landingScreen").isVisible(), true, "fresh installation must render the current shell");
  await freshContext.close();

  console.log("PASS browser service-worker fresh install, v2.7.0-alpha.13 upgrade, cache cleanup, and offline reload");
} finally {
  await browser.close();
