...
const previousWorker = currentWorker.replace("v2.7.0-alpha.2", "v2.7.0-alpha.1");
...
try {
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.1");
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.2", ["sales-capture-v2.7.0-alpha.1"]);
  ...
  await waitForCache(freshPage, "sales-capture-v2.7.0-alpha.2", ["sales-capture-v2.7.0-alpha.1"]);
  assert.equal(await freshPage.locator("#landingScreen").isVisible(), true, "fresh installation must render the current shell");
  await freshContext.close();

  console.log("PASS browser service-worker fresh install, v2.7.0-alpha.1 upgrade, cache cleanup, and offline reload");
} finally {
  await browser.close();
