...
const previousWorker = currentWorker.replace("v2.7.0-alpha.4", "v2.7.0-alpha.3");
...
try {
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.3");
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.4", ["sales-capture-v2.7.0-alpha.3"]);
  ...
  await waitForCache(freshPage, "sales-capture-v2.7.0-alpha.4", ["sales-capture-v2.7.0-alpha.3"]);
  assert.equal(await freshPage.locator("#landingScreen").isVisible(), true, "fresh installation must render the current shell");
  await freshContext.close();

  console.log("PASS browser service-worker fresh install, v2.7.0-alpha.3 upgrade, cache cleanup, and offline reload");
} finally {
  await browser.close();
