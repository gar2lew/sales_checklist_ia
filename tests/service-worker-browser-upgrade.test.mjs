...
const previousWorker = currentWorker.replace("v1.10.2", "v1.10.1");
...
try {
  ...
  await waitForCache(upgradePage, "sales-capture-v1.10.1");
  ...
  await waitForCache(upgradePage, "sales-capture-v1.10.2", ["sales-capture-v1.10.1"]);
  ...
  await waitForCache(freshPage, "sales-capture-v1.10.2", ["sales-capture-v1.10.1"]);
  assert.equal(await freshPage.locator("#landingScreen").isVisible(), true, "fresh installation must render the current shell");
  await freshContext.close();

  console.log("PASS browser service-worker fresh install, v1.10.1 upgrade, cache cleanup, and offline reload");
} finally {
  await browser.close();
