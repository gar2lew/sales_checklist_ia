...
const previousWorker = currentWorker.replace("v2.7.0-alpha.19", "v2.7.0-alpha.18");
...
try {
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.18");
  ...
  await waitForCache(upgradePage, "sales-capture-v2.7.0-alpha.19", ["sales-capture-v2.7.0-alpha.18"]);
  ...
  await waitForCache(freshPage, "sales-capture-v2.7.0-alpha.19", ["sales-capture-v2.7.0-alpha.18"]);
  assert.equal(await freshPage.locator("#landingScreen").isVisible(), true, "fresh installation must render the current shell");
  await freshContext.close();

  console.log("PASS browser service-worker fresh install, v2.7.0-alpha.18 upgrade, cache cleanup, and offline reload");
} finally {
  await browser.close();
