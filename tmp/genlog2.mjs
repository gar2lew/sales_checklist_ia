...
const clickP = await Promise.race([ page.click('#generateTop').then(()=>console.log('CLICKED'), e=>console.log('ERR '+e.message.split('\n')[0])), new Promise(r => setTimeout(() => { console.log('CLICKTIMEOUT'); r(); }, 15000)) ]);
await (async () => { for (let i = 0; i < 4; i++) { await new Promise(r => setTimeout(r, 10000)); console.log('TICK', (i+1)*10+'s'); } })();
await browser.close();
